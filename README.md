...
How It Works
    ...
    Upload: The Red Team operator uploads a payload (e.g., payload.exe).
    Processing: Nakba appends a unique, obfuscated signature to the end of the file.
    Delivery: The target is sent a link.
    Execution: The target visits the link. The browser "renders" the file (displaying a blank page or garbage text if it's a binary), but crucially, saves the clean file to the cache.
    Retrieval: A simple command on the target machine locates the payload in the cache using the signature.
...
1. Generate the Payload Link 
    ...
    Navigate to http://localhost:8080 (or your hosted domain). 
    Drag and drop your payload (e.g., implant.exe). 
...
Made for Red Teamers, by Red Teamers. 
</div>